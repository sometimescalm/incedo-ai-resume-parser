...
PROMPT_TEMPLATE = """
You are an AI bot designed to act as a professional resume parser. You are given a resume, and your task is to extract only the following top-level fields and return them in valid JSON format.
Output Requirements:
- Output must be a valid JSON object.
- Do not include triple backticks (```), markdown formatting, or labels like “json”.
- The entire JSON must be on a single line.
- If there are line breaks in any string (like professional summary or work experience), they must be escaped using \\n.
- Do not include any nested structures or extra fields.
...
- work_experience: Summarized work history and responsibilities, max 10 lines, as a single string with \\n between lines.
